                 ...
                 ulong aa, ulong bb, ulong ab, ulong ba)
{
  ...
  habbaaa[bb] += nbr_pairs;
}

void recv_hist(const ulong beta)
{
#ifdef WOC_MPI_DEBUG
  ulong tot_pairs = 0;    // count hist
#endif
  MPI_Status status;
  for (ulong nbr_end_tags = 0; nbr_end_tags != nbr_workers;)
  {
    int nbr_hist_entries;
    MPI_Recv((void*) &(entry_buffer[0]), entry_buffer_size, entry_mpi_struct_type, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
    MPI_Get_count(&status, entry_mpi_struct_type, &nbr_hist_entries);
#ifdef WOC_MPI_DEBUG
    cout << "Process " << myid << " received " << nbr_hist_entries << endl;
#endif
    for (int buff_index = 0; buff_index != nbr_hist_entries; buff_index++)
    {
    ...
    if (status.MPI_TAG == END_TAG)
    {
      ...
      cout << "Process " << myid << " received: " << setw(13) << tot_pairs << " pairs" << endl;
#endif
      nbr_end_tags++;
    }
  }
...
void send_hist()
{
        ...
        if (habba != NULL)
        {
          for (ulong aa=0; aa<N; ++aa)
          {
            ...
            if (habbaaa != NULL)
            {
              ...
              {
                const ulong nbr_pairs = habbaaa[bb];
                if (nbr_pairs)
                {
                  hist_entry* buff = &(entry_buffer[buff_index]);
                  buff->m_aa = aa;  buff->m_bb = bb;
                  buff->m_ab = ab;  buff->m_ba = ba;
                  buff->m_nbr_pairs = nbr_pairs;
                  buff_index++;
#ifdef WOC_MPI_DEBUG
                  tot_pairs += nbr_pairs;
#endif
                ...
                {
                  MPI_Send((void*) &(entry_buffer[0]), buff_index, entry_mpi_struct_type, 0, MORE_TAG, MPI_COMM_WORLD);
#ifdef WOC_MPI_DEBUG
  ...
  // if (buff_index != 0)
  {
    MPI_Send((void*) &(entry_buffer[0]), buff_index, entry_mpi_struct_type, 0, END_TAG, MPI_COMM_WORLD);
#ifdef WOC_MPI_DEBUG
    cout << "Process " << myid << " sent: " << buff_index << " after loop" << endl;
    cout << "Process " << myid << " sent:     " << setw(13) << tot_pairs << " pairs" << endl;
#endif
  }
}
...
{
  ulong tot_pairs = 0;    // count hist
  ulong tot_classes = 0;  // count nonzero values
  for (ulong ab=0; ab<N; ++ab)
  {
    const p3u hab = hist[ab];
    if (hab != NULL)
...
#ifndef TOTALS_ONLY
                  //print_bin("", ab, T);
                  cout <<   "pXY[" << myid << "," << tot_classes << "]:=" << poly(T, ab) << ":";
                  //print_bin("", ba, T);
                  cout <<   "pYX[" << myid << "," << tot_classes << "]:=" << poly(T, ba) << ":";
                  //print_bin("", N+aa, T);
                  cout <<   "pXX[" << myid << "," << tot_classes << "]:=" << poly(T, N+aa) << ":";
                  //print_bin("", N+bb, T);
                  cout <<   "pYY[" << myid << "," << tot_classes << "]:=" << poly(T, N+bb) << ":";
                  cout << "count[" << myid << "," << tot_classes << "]:=" << nbr_pairs << ":" << endl;
#endif
                }
              }
          }
                                ...
                                const ulong* rgs_begin,const ulong* rgs_end)
{
  ...
  for (more=true; more; more=p.next())
  {
    ...
    if (a != b)
    {
      ...
      insert_hist(beta, orbit_size, aa, bb, ab, ba);
#ifdef WOC_MPI_DEBUG
      tot_pairs += orbit_size;
#endif
    }
  }
#ifdef WOC_MPI_DEBUG
  cout << "Process " << myid << " inserted: " << setw(13) << tot_pairs << " pairs" << endl;
#endif
  return steps;
}
...
main(int argc, char **argv)
{
  ...
  if (myid == 0)
  {
    ...
    cout << "nbr_workers==" << setw(20) << nbr_workers
         << " (number of worker processes to use)" << endl;
  #ifdef WOC_DEBUG
