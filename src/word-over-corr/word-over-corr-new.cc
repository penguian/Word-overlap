...
void insert_hist(const p4u hist, const ulong N, const ulong T, const ulong log2_alpha, const ulong beta, const ulong orbit_size, const ulong m0, const ulong a, const ulong b, ulong olda, ulong oldaa, ulong ab, ulong ba)
{
                                ...
                                const ulong* rgs_begin,const ulong* rgs_end, const p4u hist)
{
  ...
  ulong olda = 0;
  ulong oldaa = autocorr(log2_alpha, m0, olda);
  ulong partitions = 0;
  for (bool more=true; more; more=p.next())
  {
    ...
    if (a != olda)
    {
      oldaa = autocorr(log2_alpha, m0, a);
      olda = a;
    }
    if (a!=b)
    {
...
main(int argc, char **argv)
{
  ...
  for (ulong beta=2; beta<=alpha; ++beta)
  {
    ...
    if (beta > max_ranges)
    {
      ...
      partitions = check_setpart_p_rgs_range(T, log2_alpha, m0, N, beta, orbit_size, rgs_begin, rgs_end, hist);

      delete [] rgs_begin;
      delete [] rgs_end;
    }
    else
      for (ulong gamma=1; gamma <= beta; gamma++)
      {
        ...
        const ulong steps = check_setpart_p_rgs_range(T, log2_alpha, m0, N, beta, orbit_size, rgs_begin, rgs_end, hist);
